// Source: https://reactbits.dev/r/Strands-JS-CSS.json
// Local adaptation: explicit React import for this Vite JSX configuration.
...
void main() {
  ...
  // Dark smoked glass: cool upper-left light falling into a charcoal base.
  vec2 sphere = p / max(r, 1e-4);
  float gradient = clamp(0.48 + sphere.y * 0.3 - sphere.x * 0.2, 0.0, 1.0);
  vec3 body = mix(vec3(0.012, 0.016, 0.028), vec3(0.12, 0.15, 0.20), gradient);
  body *= 1.0 - 0.42 * pow(nd, 2.0);
  float softLight = exp(-dot(sphere - vec2(-0.32, 0.40), sphere - vec2(-0.32, 0.40)) * 10.0);
  body += vec3(0.10, 0.12, 0.15) * softLight;
  float bodyA = 0.88 + fres * 0.10;

  // Keep the animated emissive strands intact over the shaded body.
  float outA = emissiveA + bodyA * (1.0 - emissiveA);
  vec3 outRGB = emissive + body * bodyA * (1.0 - emissiveA);
