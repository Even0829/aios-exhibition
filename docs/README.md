...
- **引元产品与通用底座**：先读 `knowledge/products/origin.md`；对象、运行机制、决策、调度与闭环读 `knowledge/system/origin-system-model.md`；来源核对读 `knowledge/sources/origin-product-base.md`。

- **AIOS 家庭康养陈展电视端 Demo**：先读 `knowledge/products/aios-exhibition-context.md`；全局状态、中断恢复和功能结构读 `architecture/aios-exhibition-experience.md`；单个体验故事从 `architecture/exhibition-stories/README.md` 路由到对应 Owner；电视通用、特殊、例外、Figma 还原、Mock、组件、动效和验收规范统一读 `specs/aios-exhibition-design-system.md`。现有产品业务场景不作为默认前置。
- 开展设备中心产品、跨端接续或开发评审：读取 `specs/device-center-prd.md`；电视设计读取 `specs/aios-exhibition-design-system.md`，手机小程序设计读取 `specs/device-center-mobile-design-system.md`。只加载当前任务对应端的设计系统，跨端一致性任务才同时读取两份。
...
- 涉及其他产品关系时，按需核对 `knowledge/system/aios-ecosystem.md` 及更新来源；旧生态不是默认阅读前置。
...
| AIOS 生态参考 | `docs/knowledge/system/aios-ecosystem.md` | 跨产品问题按需核对 | Evolving | 旧来源背景与适用边界 |
| 引元 | `docs/knowledge/products/origin.md` | 通用底座、垂类产品与陈展 | Long-term | 产品定义、职责、盒子角色与垂类边界 |
| 引元系统模型 | `docs/knowledge/system/origin-system-model.md` | 各垂类产品与系统设计 | Evolving | 对象、循环、决策三维、任务、能力与闭环 |
| 引元家庭康养产品 | `docs/knowledge/products/origin-home-eldercare.md` | 康养产品设计、架构、规格与任务 | Evolving | 康养定义、首期架构、领域原则与评价框架 |
...
| 陈展电视端设计系统与还原规范 | `docs/specs/aios-exhibition-design-system.md` | 全部电视陈展故事、Figma 还原、Mock、组件开发与验收 | Evolving | 电视基础视觉、通用组件、故事变体、特殊状态、例外、语音、动效、前端映射与验收的唯一完整 Owner |
| 设备中心手机小程序设计系统 | `docs/specs/device-center-mobile-design-system.md` | 手机设备中心视觉设计、组件、动效、交互和页面还原 | Evolving | 手机设备中心设计与还原规范的唯一 Owner |
| 设备中心 PRD | `docs/specs/device-center-prd.md` | 设备中心电视端、手机小程序、跨端接续和开发评审 | Evolving | 共用对象、状态、电视与手机功能、品牌接入、远程控制、消息、AI、最小产品服务端边界、扩展性与验收的产品需求 Owner |
| 引元产品知识底座来源 | `docs/knowledge/sources/origin-product-base.md` | Source intake 与来源核对 | Evolving | 新来源优先级、修订和吸收范围 |
