...
该文件是本项目的第一手参考输入和追溯来源，但不是本仓库的 Durable Owner。被确认并采用的内容必须吸收到对应 Owner，后续设计优先引用 Owner，不直接依赖整份原始材料。
...
- 日常产品设计不默认加载原始资料。
- Owner 缺少必要背景、出现定义冲突或开展新一轮 source intake 时，才回到原始资料核查。
- 原始资料中与当前项目无直接依赖的官网、开发者、商业、合作伙伴和其他行业场景内容暂不吸收。
- 原始资料中的未确认状态、公开范围、部署规模、量化指标和完整经济规则不得提升为项目事实。
